...
def run_repeated_training(base_config_path: str, seeds: list[int], device: str) -> dict:
    ...
    temp_dir = Path("outputs/repeat_runs_edge_aware")
    temp_dir.mkdir(parents=True, exist_ok=True)

    for seed in seeds:
        config = copy.deepcopy(base_config)
        config["seed"] = seed
        config["paths"]["output_dir"] = f"outputs/research_edge_aware_seed{seed}"
    ...
    out_path = Path("outputs/research_edge_aware_summary.json")
