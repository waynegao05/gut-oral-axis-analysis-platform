...
def build_tabular_dataframe(config: dict) -> tuple[pd.DataFrame, dict]:
    ...
    graph_features = abundance_pivot.join(function_pivot, how="outer").join(edge_summary, how="outer").reset_index()
    ...
    feature_groups = {
        ...
        "graph_summary": [col for col in merged.columns if col.startswith("abundance__") or col.startswith("function__") or col.startswith("edge_weight_") or col in {"num_edges", "num_nodes", "graph_density"}],
    }
    feature_groups["clinical_metabolite"] = feature_groups["clinical"] + feature_groups["metabolite"]
    feature_groups["all_tabular"] = feature_groups["clinical"] + feature_groups["metabolite"] + feature_groups["graph_summary"]
    return merged, feature_groups


def split_dataframe(df: pd.DataFrame, seed: int, val_ratio: float, test_ratio: float) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
...
def prepare_split_data(train_df: pd.DataFrame, val_df: pd.DataFrame, test_df: pd.DataFrame, feature_columns: List[str]) -> SplitData:
...
def evaluate_model(model: torch.nn.Module, split: SplitData, which: str = "val") -> dict:
    ...
    return {"loss": float(loss), "c_index": float(c_index), "risk": risk_np.tolist()}
...
def run_baseline_suite(config_path: str, seeds: List[int]) -> dict:
    ...
    baseline_specs = {
        "clinical_linear_cox": {"features": feature_groups["clinical"], "model_type": "linear"},
        "metabolite_linear_cox": {"features": feature_groups["metabolite"], "model_type": "linear"},
        "clinical_metabolite_linear_cox": {"features": feature_groups["clinical_metabolite"], "model_type": "linear"},
        "graph_summary_linear_cox": {"features": feature_groups["graph_summary"], "model_type": "linear"},
        "all_tabular_linear_cox": {"features": feature_groups["all_tabular"], "model_type": "linear"},
        "clinical_metabolite_mlp_cox": {"features": feature_groups["clinical_metabolite"], "model_type": "mlp"},
        "all_tabular_mlp_cox": {"features": feature_groups["all_tabular"], "model_type": "mlp"},
    }
...
def main() -> None:
    ...
    parser.add_argument("--config", default="research_config_small_sample.yaml")
