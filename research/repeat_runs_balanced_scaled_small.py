...
def run_repeated_training(
    base_config_path: str = "research_config_balanced_scaled_small.yaml",
    ...
    temp_dir = Path("outputs/repeat_runs_balanced_scaled_small")
    temp_dir.mkdir(parents=True, exist_ok=True)

    for seed in seeds:
        config = copy.deepcopy(base_config)
        config["seed"] = seed
        config["paths"]["output_dir"] = f"outputs/research_balanced_scaled_small_seed{seed}"
        ...
        cmd = [
            "python",
            "-m",
            "research.train_balanced_scaled",
            "--config",
    ...
    out_path = Path("outputs/research_balanced_scaled_small_summary.json")
