...
$experimentName = "topology_v7_internal_relation_site_v3"
...
function Invoke-Runner {
    ...
    $arguments = @(
        "-m", "experiments.topology_v7_internal_relation_site_v3.runner",
        $Phase,
...
try {
    ...
    if ($Mode -eq "audit" -or $Mode -eq "full") {
        ...
        if (-not (Test-Path -LiteralPath $resolvedLockPath)) {
            throw "Development must complete before audit generation."
        }
        ...
        if ($lock.status -ne "locked_after_development_before_audit_generation") {
            throw "No new candidate passed the development gate; audit generation is prohibited."
        }
