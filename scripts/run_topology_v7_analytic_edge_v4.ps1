...
$experimentName = "topology_v7_analytic_edge_v4"
...
function Invoke-Runner {
    ...
    $arguments = @(
        "-m", "experiments.topology_v7_analytic_edge_v4.runner",
        $Phase,
...
try {
    ...
    if ($Mode -eq "audit" -or $Mode -eq "full") {
        ...
        if (-not (Test-Path -LiteralPath $resolvedLockPath)) {
            throw "V4 development must complete before audit generation."
        }
        ...
        if ($lock.status -ne "locked_after_development_before_audit_generation") {
            throw "No V4 candidate passed the performance gate; audit generation is prohibited."
        }
