...
$experimentName = "topology_v7_rank_objective_v6"
...
function Invoke-Runner {
    ...
    $arguments = @(
        "-m", "experiments.topology_v7_rank_objective_v6.runner",
        $Phase,
        "--data-dir", $DataDirectory,
...
try {
    ...
    if ($Mode -eq "audit" -or $Mode -eq "full") {
        ...
        if (-not (Test-Path -LiteralPath $resolvedLockPath)) {
            throw "V6 development must complete before audit generation."
        }
        ...
        if ($lock.status -ne "locked_after_development_before_audit_generation") {
            throw "No V6 candidate passed the development gate; audit generation is prohibited."
        }
