...
from torch_geometric.nn import GATConv, GlobalAttention, global_add_pool, global_max_pool, global_mean_pool


def _compute_structure_features(batch):
    ...
    for graph_id in range(num_graphs):
        ...
        if local_n == 0:
            graph_targets.append(torch.zeros(4, device=device))
            continue
        ...
        graph_target = torch.stack(
            [
                local_clustering.mean(),
                (wadj.sum(dim=1).mean() / max(float(local_n), 1.0)),
                local_bridge.mean() / max(float(local_n), 1.0),
                tri_diag.mean() / max(float(local_n), 1.0),
            ]
        )
    ...
    graph_target_tensor = torch.stack(graph_targets)
    return node_struct, graph_target_tensor


class StructureAwareGATCoxV2(nn.Module):
    def __init__(
        self,
        ...
        self.graph_struct_proj = nn.Sequential(
            nn.Linear(4, hidden_dim // 2),
            nn.ReLU(),
            nn.Dropout(dropout),
        )
        self.graph_branch_proj = nn.Sequential(
            nn.Linear(hidden_dim * 4 + hidden_dim // 2, hidden_dim),
            nn.ReLU(),
            nn.Dropout(dropout),
        )
        self.graph_only_head = nn.Linear(hidden_dim, 1)

        self.clinical_proj = nn.Sequential(nn.Linear(clinical_dim, hidden_dim // 2), nn.ReLU())
        self.metabolite_proj = nn.Sequential(nn.Linear(metabolite_dim, hidden_dim // 2), nn.ReLU())
        self.fusion_gate = nn.Sequential(
            nn.Linear(hidden_dim + hidden_dim // 2 + hidden_dim // 2, 3),
            nn.Softmax(dim=1),
        )
    ...
    def forward(self, batch):
        node_struct, graph_target = _compute_structure_features(batch)
        ...
        add_pool = global_add_pool(h3, batch.batch)
        att_pool = self.att_pool(h3, batch.batch)
        graph_struct = self.graph_struct_proj(graph_target)
        graph_concat = torch.cat([mean_pool, max_pool, add_pool, att_pool, graph_struct], dim=1)
        graph_branch = self.graph_branch_proj(graph_concat)
        graph_only_risk = self.graph_only_head(graph_branch).squeeze(-1)

        clinical = self.clinical_proj(batch.clinical.view(graph_branch.size(0), -1))
        metabolites = self.metabolite_proj(batch.metabolites.view(graph_branch.size(0), -1))
        gates = self.fusion_gate(torch.cat([graph_branch, clinical, metabolites], dim=1))
        gated_graph = graph_branch * gates[:, 0:1]
        gated_clinical = clinical * gates[:, 1:2]
        gated_metabolites = metabolites * gates[:, 2:3]
        fused = torch.cat([gated_graph, gated_clinical, gated_metabolites], dim=1)
        latent = self.fusion(fused)
        risk = self.risk_head(latent).squeeze(-1)

        structure_pred = self.structure_head(graph_branch)
        aux_loss = self.aux_loss_fn(structure_pred, graph_target)
        return {
            "risk": risk,
            "graph_only_risk": graph_only_risk,
