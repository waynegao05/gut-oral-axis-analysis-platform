...
def run_repeated_training(
    base_config_path: str = "research_config_balanced_scaled_mid.yaml",
    ...
    temp_dir = Path("outputs/repeat_runs_balanced_scaled_mid")
    temp_dir.mkdir(parents=True, exist_ok=True)

    for seed in seeds:
        config = copy.deepcopy(base_config)
        config["seed"] = seed
        config["paths"]["output_dir"] = f"outputs/research_balanced_scaled_mid_seed{seed}"
    ...
    out_path = Path("outputs/research_balanced_scaled_mid_summary.json")
