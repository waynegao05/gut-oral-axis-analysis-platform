...
def run_repeated_training(base_config_path: str, seeds: list[int]) -> dict:
    ...
    for seed in seeds:
        ...
        config["seed"] = seed
        config["paths"]["output_dir"] = f"outputs/research_small_sample_seed{seed}"
    ...
    out_path = Path("outputs/research_small_sample_summary.json")
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        default="research_config_small_sample.yaml",
        help="Base config file used for repeated training.",
    )
    parser.add_argument(
        "--seeds",
        nargs="+",
        type=int,
        default=[7, 21, 42, 123, 2026],
        help="One or more random seeds.",
    )
    args = parser.parse_args()

    result = run_repeated_training(args.config, args.seeds)
