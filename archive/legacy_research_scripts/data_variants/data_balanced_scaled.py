...
def _fit_standardizer(df: pd.DataFrame, columns: List[str], train_idx: List[int]) -> Tuple[pd.Series, pd.Series]:
    train_df = df.iloc[train_idx]
    mean = train_df[columns].mean()
    std = train_df[columns].std().replace(0, 1.0).fillna(1.0)
    return mean, std


def _apply_standardizer(df: pd.DataFrame, columns: List[str], mean: pd.Series, std: pd.Series) -> pd.DataFrame:
    out = df.copy()
    out[columns] = (out[columns] - mean) / std
    return out
...
def build_dataset_from_csv(
    ...
    sample_order = sorted(graph_df["sample_id"].unique().tolist())
    ...
    clinical_mean, clinical_std = _fit_standardizer(meta_for_split, clinical_columns, train_idx)
    metabolite_mean, metabolite_std = _fit_standardizer(meta_for_split, metabolite_columns, train_idx)
    scaled_meta = _apply_standardizer(meta_for_split, clinical_columns, clinical_mean, clinical_std)
    scaled_meta = _apply_standardizer(scaled_meta, metabolite_columns, metabolite_mean, metabolite_std)

    data_list: List[Data] = []
    for sample_id, sample_graph in graph_df.groupby("sample_id"):
        sample_meta = scaled_meta.loc[scaled_meta["sample_id"] == sample_id]
