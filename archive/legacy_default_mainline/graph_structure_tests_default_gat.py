...
def perturb_graph_dataframe(graph_df: pd.DataFrame, variant: str, seed: int) -> pd.DataFrame:
    ...
    for sample_id, sample_graph in graph_df.groupby("sample_id"):
        ...
        if variant == "shuffle_edges_and_weights":
            ...
            while len(new_edges) < num_edges:
                src = node_names[len(new_edges) % len(node_names)]
                dst = node_names[(len(new_edges) + 2) % len(node_names)]
                if src != dst:
                    new_edges.append((src, dst))
...
def run_graph_structure_tests(config_path: str, seeds: List[int], variants: List[str]) -> dict:
    ...
    for variant in variants:
        for seed in seeds:
            ...
            cmd = ["python", "-m", "research.train", "--config", str(temp_config_path)]
...
def main() -> None:
    ...
    result = run_graph_structure_tests(args.config, args.seeds, args.variants)
