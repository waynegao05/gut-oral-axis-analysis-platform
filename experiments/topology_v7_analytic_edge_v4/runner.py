...
DEFAULT_OUTPUT_ROOT = ROOT / "outputs/topology_v7_analytic_edge_v4"
...
def _build_candidate_model(
    *,
    ...
    if candidate["model_family"] != "analytic_internal_relation":
        raise ValueError(
            f"Unsupported V4 model family: {candidate['model_family']}"
        )
    parameters = fit_analytic_edge_parameters(
        bundle.train_set,
        num_node_types=bundle.num_node_types,
    )
    model = AnalyticInternalRelationModel(
        base_model,
        num_node_types=bundle.num_node_types,
        site_feature_dim=bundle.site_feature_dim,
        parameters=parameters,
        use_linear_site_residual=bool(
            candidate["use_linear_site_residual"]
        ),
    ).to(device)
    ...
    *,
    ...
    if (
        ...
        if result.get("run_fingerprint") != fingerprint:
            raise RuntimeError(
                f"Refusing mismatched V4 resume artifacts: {output_dir}"
            )
        return result
        ...
        selection_history.append(
            {
                ...
                "site_residual_penalty": float(penalty.item()),
    ...
    if best_state is None:
        raise RuntimeError("V4 selection produced no checkpoint.")
...
def run_development(
    *,
    ...
    if plan["status"] != "locked_before_development_generation":
        raise RuntimeError("V4 development plan is not locked.")
    ...
    if int(manifest["seed"]) != int(plan["development_generation_seed"]):
        raise RuntimeError("V4 development seed does not match the plan.")
    ...
    summary = {
        "schema_version": 1,
        "status": "complete",
        "scope": "fresh_analytic_edge_nested_logo_development",
        "plan_sha256": core._sha256(plan_path),
...
def run_audit(
    *,
    ...
    if (
        ...
        raise RuntimeError("No V4 candidate is eligible for audit.")
    manifest_path = data_dir / core.DATA_FILE_NAMES["manifest_json"]
    manifest = core._read_json(manifest_path)
    if int(manifest["seed"]) != int(lock["audit_generation_seed"]):
        raise RuntimeError("V4 audit seed does not match the lock.")
    candidate_by_name = {
    ...
    summary = {
        "schema_version": 1,
        "status": "complete",
        "scope": "one_time_analytic_edge_nested_logo_audit",
        "plan_sha256": core._sha256(plan_path),
        "manifest_sha256": core._sha256(manifest_path),
        "aggregates": aggregates,
